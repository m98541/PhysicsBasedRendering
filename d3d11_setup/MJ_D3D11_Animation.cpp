	...
	for (uint32_t i = 0; i < targetSkeleton->jointsData->count ; ++i)
	{
		...
		if (quatKeySize > 0)
		{
			...
			for (uint32_t j = searchTimeLineQuatRotIndex; j + 1 < quatKeySize; ++j)
			{
				...
				quatInterVal.y = curAnimationClip->AnimationJoint[i].quatRotKeys[j + 1].time / totalTime;

				if (t >= quatInterVal.x && t <= quatInterVal.y)
				{
					searchTimeLineQuatRotIndex = (searchTimeLineQuatRotIndex + j) % (quatKeySize - 1);
					break;
				}
			}
			...
			{
				DirectX::XMVECTOR quat0 = DirectX::XMLoadFloat4(&curAnimationClip->AnimationJoint[i].quatRotKeys[searchTimeLineQuatRotIndex].quatRot);
				DirectX::XMVECTOR quat1 = DirectX::XMLoadFloat4(&curAnimationClip->AnimationJoint[i].quatRotKeys[searchTimeLineQuatRotIndex + 1].quatRot);
				quat0 = DirectX::XMVectorScale(quat0, (1 - quatInterpCoeffs));
				quat1 = DirectX::XMVectorScale(quat1, (quatInterpCoeffs));
				DirectX::XMStoreFloat4(&targetSkeleton->jointsData->array[i].jointLocalPose.quatRot, DirectX::XMVectorAdd(quat0, quat1));
			}
		...
		if (transKeySize > 0)
		{
			...
			for (uint32_t j = searchTimeLineTransIndex; j + 1 < transKeySize; ++j)
			{
				...
				transInterVal.y = curAnimationClip->AnimationJoint[i].transKeys[j + 1].time / totalTime;

				if (t >= transInterVal.x && t <= transInterVal.y)
				{
					searchTimeLineTransIndex = (searchTimeLineTransIndex + j) % (transKeySize - 1);
					break;
				}
			}
		...
		if (scaleKeySize > 1)
		{
			...
			for (uint32_t j = searchTimeLineScaleIndex; j + 1 < scaleKeySize; ++j)
			{
				...
				scaleInterVal.y = curAnimationClip->AnimationJoint[i].scaleKeys[j + 1].time / totalTime;
			...
			else
			{
				...
				DirectX::XMVECTOR scale0 = DirectX::XMLoadFloat3(&curAnimationClip->AnimationJoint[i].scaleKeys[searchTimeLineScaleIndex].scale);
				DirectX::XMVECTOR scale1 = DirectX::XMLoadFloat3(&curAnimationClip->AnimationJoint[i].scaleKeys[searchTimeLineScaleIndex + 1].scale);
